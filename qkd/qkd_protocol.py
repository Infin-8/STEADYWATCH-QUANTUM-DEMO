...
from .ldpc_error_correction import LDPCErrorCorrection, create_ldpc_corrector
from .quantum_amplified_ldpc import QuantumAmplifiedLDPC, create_quantum_amplified_ldpc
from .cascade_key_reconciliation import CascadeProtocol, create_cascade_protocol
...
class QKDProtocol:
    ...
    def __init__(self,
        ...
        # Initialize LDPC error correction (advanced)
        self.ldpc_corrector = create_ldpc_corrector(code_rate=0.5, code_length=256)
        
        # Initialize Quantum-Amplified LDPC (Echo Resonance)
        self.quantum_ldpc = create_quantum_amplified_ldpc(
            code_rate=0.5,
            code_length=256,
            use_echo_resonance=True
        )
        
        # Initialize Cascade protocol (iterative binary search)
        self.cascade_protocol = create_cascade_protocol(block_size=8, num_passes=4)
    ...
    def error_correction_repetition(self, raw_key: bytes, 
        ...
        """
        Correct errors using repetition codes (legacy method).
        ...
        Returns:
            Corrected key
        """
        print(f"🔧 [{self.party_id}] Correcting errors (repetition codes)...")
    ...
    def error_correction_ldpc(self, key_alice: bytes, key_bob: bytes,
                             error_rate: float) -> Tuple[bytes, bytes, Dict]:
        """
        Correct errors using LDPC (Low-Density Parity-Check) codes.
        
        More efficient than repetition codes with lower overhead.
        ...
        """
        print(f"🔧 [{self.party_id}] Correcting errors using LDPC codes...")
        
        corrected_alice, corrected_bob, metadata = self.ldpc_corrector.correct_errors(
            key_alice=key_alice,
            key_bob=key_bob,
            error_rate=error_rate
        )
        
        self.corrected_key = corrected_alice if self.party_id == "alice" else corrected_bob
        
        print(f"✅ [{self.party_id}] LDPC error correction complete")
        
        return corrected_alice, corrected_bob, metadata
    
    def error_correction_quantum_amplified(self, key_alice: bytes, key_bob: bytes,
                                         error_rate: float) -> Tuple[bytes, bytes, Dict]:
        """
        Correct errors using Quantum-Amplified LDPC (Echo Resonance).
        
        Uses Echo Resonance to generate parity bits quantum-mechanically:
        - Message bits → Master qubit(s)
        - Echo Resonance → Parity bits (satellites)
        ...
        Returns:
            Tuple of (corrected_key_alice, corrected_key_bob, correction_metadata)
        """
        print(f"🔬 [{self.party_id}] Correcting errors using Quantum-Amplified LDPC (Echo Resonance)...")
        
        corrected_alice, corrected_bob, metadata = self.quantum_ldpc.correct_errors_quantum(
            key_alice=key_alice,
            key_bob=key_bob,
            error_rate=error_rate
        )
        ...
        print(f"✅ [{self.party_id}] Quantum-amplified LDPC error correction complete")
        print(f"   Quantum parity generations: {metadata.get('quantum_parity_generated', 0)}")
        
        return corrected_alice, corrected_bob, metadata
    
    def error_correction_cascade(self, key_alice: bytes, key_bob: bytes,
                                error_rate: float) -> Tuple[bytes, bytes, Dict]:
        """
        Correct errors using Cascade protocol (iterative binary search).
        
        The Cascade protocol:
        1. Divides key into blocks
        2. Compares parity of blocks
        3. Uses binary search to locate errors
        4. Iteratively refines error locations across multiple passes
        ...
        Returns:
            Tuple of (corrected_key_alice, corrected_key_bob, correction_metadata)
        """
        print(f"🔄 [{self.party_id}] Correcting errors using Cascade protocol...")
        
        reconciled_alice, reconciled_bob, metadata = self.cascade_protocol.reconcile_keys(
            key_alice=key_alice,
            key_bob=key_bob,
            error_rate=error_rate
        )
        
        self.corrected_key = reconciled_alice if self.party_id == "alice" else reconciled_bob
        
        print(f"✅ [{self.party_id}] Cascade reconciliation complete")
        print(f"   Total errors corrected: {metadata['total_errors_corrected']}")
        print(f"   Keys match: {metadata['keys_match']}")
        
        return reconciled_alice, reconciled_bob, metadata
