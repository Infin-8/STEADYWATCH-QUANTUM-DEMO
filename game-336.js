...
(function () {
    ...
    function createBlockMesh(blockType, bx, by, bz) {
        ...
        if (blockType === BLOCK.GROUND) {
            ...
            material = new THREE.MeshPhysicalMaterial({
                color: new THREE.Color().setHSL(0.52, 0.85, levelBright),
                emissive: new THREE.Color().setHSL(0.52, 1.0, 0.35),
    ...
    function initGame() {
        ...
        camera.position.set(5.00, 0.83, -5.00); 
        camera.lookAt(0, 5, 0);
        ...
        var beaconFlares = [
            ...
            { sc: 5.5, r: 0,   g: 229, b: 255, op: 0.40 },
            { sc: 9.5, r: 0,   g: 229, b: 255, op: 0.12 }
        ].map(function(d) {
        ...
        function buildHurwitzCrystalLayer() {
            var quats = window.HurwitzKeys ? window.HurwitzKeys.unzipSeed(13) : [];
            var crystalGeom = new THREE.SphereGeometry(0.10, 8, 8);
            // Board-aligned distribution: snap each node to its nearest 9x9 cell.
            // p=13 components span [-3,3] so 49 of 81 cells are populated (~6.9 nodes/cell).
            var cellCounts = {};
        ...
        function buildWorld() {
            ...
            // Hurwitz quaternion lattice nodes — 336 sites projected from 4D (p=13), floating above crystal floor
        ...
        function animate() {
            ...
            for (idx = 0; idx < blockMeshes.length; idx++) {
                ...
                if (m.userData.blockType === BLOCK.PYRAMID_ORE) {
                    ...
                    m.material.emissive.setHSL(0.52 + pPulse * 0.02, 1.0, 0.3 + pPulse * 0.1);
                    continue;
                }
        ...
        window.toggleFingerprintView336 = function () {
            ...
            if (fingerprintMode336) {
                ...
                if (window.HurwitzLatticeAuth) {
                    window.HurwitzLatticeAuth.getFingerprint(13).then(function (fp) {
                        var label = document.getElementById('game-key-label');
                        if (label) label.textContent = 'Fingerprint: p=13 · ' + fp.siteCount + ' sites · ID: ' + fp.shortId;
                    });
                }
            } else {
                ...
                controls.update();
                var label = document.getElementById('game-key-label');
                if (label) label.textContent = 'Click key ore to mine. Each drop shows 336 keyz in four arms (84 per arm).';
            }
        };
